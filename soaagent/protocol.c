 ...
 */
static int crc32_check(message_header * header, const void * body, size_t body_len) {
    int ret = 0;
...
int send_message(int fd, uint32_t identifier, uint8_t type, const void * buffer, size_t buf_len) {
    ...
    header->type = type;
    uint32_t crc = crc32(message_buf + 4, message_len - 4);
    //INFO("--------------crc value  : %u", crc);
    header->crc = htonl(crc);
...
int receive_message(int fd, uint32_t * identifier, uint8_t * type, void ** out_buf, size_t * out_buf_len) {
    ...
    *identifier = ntohl(header.identifier);
    ...
    char * body_value = (char *) malloc(body_len);
